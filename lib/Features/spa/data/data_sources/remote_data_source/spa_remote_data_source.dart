...
typedef SpaResponse = Either<String, SpaList>;

abstract class SpaRemoteDataSource {
  Future<SpaResponse> fetchAllSpaServices();
}

class SpaRemoteDataSourceImpl extends SpaRemoteDataSource {
  @override
  Future<SpaResponse> fetchAllSpaServices() async {
    SpaResponse spaResponse = left("");
    ...
    await getIt<NetworkRequest>().requestFutureData<SpaModelResponse>(
      ...
      onSuccess: (data) {
        print('=== Spa Services onSuccess Debug ===');
        ...
        if (data.data != null && data.data is List) {
          ...
          print('❌ Failed to fetch spa services');
          spaResponse = left(data.message ?? "فشل جلب البيانات");
        }
      },
      onError: (code, msg) {
        spaResponse = left(msg.toString());
      },
    );
    return spaResponse;
  }
